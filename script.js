// 中青年人群肥胖风险预测（研究原型）
// 模型：RandomForest（n_estimators=300, max_depth=10, min_samples_leaf=4）
// 特征：EFA单因子膳食模式得分(m1_score) + 运动习惯 + 肥胖家族史 + EBQ总分 + 吸烟状态
...
// 由训练数据用 m1_score ~ 13个食物维度均值得分 做OLS精确还原（R²≈1.000）
const M1_INTERCEPT = -3.87846085073198;
const M1_COEF = {
  grp_staple: 0.080745, grp_tuber: 0.068714, grp_pickled_fried: 0.077931, grp_egg: 0.063413,
  grp_meat: 0.069911, grp_seafood: 0.086241, grp_dairy: 0.075712, grp_snack: 0.083505,
  grp_beverage: 0.038139, grp_soy: 0.128228, grp_vegetable: 0.085380, grp_fruit: 0.130926,
  grp_dried: 0.097799,
};
...
function computeM1Score(grpValues) {
  let s = M1_INTERCEPT;
  for (const g of GRP_DEFS) s += M1_COEF[g.key] * grpValues[g.key];
  return s;
}
...
function predict(inputs) {
  // inputs: { grp_* (1-9) x13, exercise(1|2), fam_obesity(0|1), smoking(1|2|3), ebq_total }
  const m1_score = computeM1Score(inputs);
  const rawByCol = { m1_score, exercise: inputs.exercise, fam_obesity: inputs.fam_obesity, ebq_total: inputs.ebq_total };
...
function renderResult(res) {
  ...
  box.innerHTML = `
    <div class="result-card ${riskClass}">
      ...
      <div class="result-sub">判别阈值：${(MODEL.best_threshold * 100).toFixed(1)}%（训练集OOF Youden指数确定）｜ 膳食模式得分：${res.m1_score.toFixed(2)}</div>
    </div>`;
